...
deactivate (NMDevice *device)
{
	...
	nm_platform_wifi_set_powersave (nm_device_get_platform (device), ifindex, 0);
                          ...
                          NMAccessPoint *ap)
{
	...
	if (g_strcmp0 (band, "a") == 0)
		freq = nm_platform_wifi_find_frequency (nm_device_get_platform (NM_DEVICE (self)), nm_device_get_ifindex (NM_DEVICE (self)), a_freqs);
	else
		freq = nm_platform_wifi_find_frequency (nm_device_get_platform (NM_DEVICE (self)), nm_device_get_ifindex (NM_DEVICE (self)), bg_freqs);
	...
	if (priv->current_ap) {
		if (nm_ap_get_fake (priv->current_ap)) {
			...
			if (!nm_ap_get_freq (priv->current_ap))
				nm_ap_set_freq (priv->current_ap, nm_platform_wifi_get_frequency (nm_device_get_platform (device), ifindex));
			if (!nm_ap_get_max_bitrate (priv->current_ap))
				nm_ap_set_max_bitrate (priv->current_ap, nm_platform_wifi_get_rate (nm_device_get_platform (device), ifindex));
		}
