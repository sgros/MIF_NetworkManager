...
device_link_changed (NMDevice *self)
{
	...
	if (priv->ifindex > 0 && !priv->platform_link_initialized && info.initialized) {
		...
		if (nm_platform_link_get_unmanaged (nm_device_get_platform(self), priv->ifindex, &platform_unmanaged)) {
...
static void
impl_device_change_network_namespace (NMDevice *self,
                                      GDBusMethodInvocation *context,
...
nm_device_class_init (NMDeviceClass *klass)
{
	...
	nm_exported_object_class_add_interface (NM_EXPORTED_OBJECT_CLASS (klass),
	                                        ...
	                                        "ChangeNetworkNamespace", impl_device_change_network_namespace,
	                                        NULL);
}
